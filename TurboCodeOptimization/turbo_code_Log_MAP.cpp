...
int *LLR_all_turbo_scr;
int *LLR_all_turbo_scr_tmp;
...
void TurboCodingInit(int length_info)
{
	...
	cout<<"@@@@@@@@@@@@@@ M_num_reg     :" <<M_num_reg<<"\n";
	cout<<"@@@@@@@@@@@@@@ Length total    : " << length_total << "\n";
	...
	cout<<"Printing the generator matrix\n";
	cout<< *(turbo_g.g_matrix);
	...
	cout<<"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ \n";
	cout<<sizeof(index_randomintlvr) << "  :  "<< sizeof(reverse_index_randomintlvr) <<"\n";

    //index_randomintlvr = (int *)malloc(sizeof(int));
    //reverse_index_randomintlvr = (int *)malloc(sizeof(int));

    //cout<< "@@@@@@@@@@@  Interleaver and Reverse interleaver indices @@@@@@@@@@@@@ : " << *(index_randomintlvr) << "  :  "<<*(reverse_index_randomintlvr);
						...
						int *traffic_source_length)
{
	...
	//cout<<"\n ********************************** send "<< (3*length_info+3*(M_num_reg +1))*sizeof(int) <<"\n";
						  ...
						  int *trafficflow_length, float EbN0dB)
{
    ...
    cout << "@@@@@@@@@@@@@@   Length total : "<< length_total << "   :  LengthInfo    : "<<length_info <<"\n";

    int LLR_prev_turbo[length_total];
    ...
    LLR_all_turbo_scr_tmp=(int *)malloc(length_total*sizeof(int));

    LLR_all_turbo_scr=(int *)malloc(length_total*sizeof(int));

	tempout=(int *)malloc(length_total*sizeof(int));

    cout<< "################# Commencing depuncturing ############\n";
	...
	if (TURBO_PUNCTURE)		/* fill in the punctured bits and demultiplex */
	{
		...
		cout<<"############# Depuncturing complete ###############\n";
		demultiplex(receive_punc, length_total, yk_turbo);
		cout<<"############ Demultiplexing complete ###############\n";
	}
	else
	{
	...
	cout<<"#################### Depuncturing check complete ############# \n";
	/*	scale the data	*/
    ...
    cout<<"############# Commencing the Iterations ############## \n";
	for (iteration=0; iteration<N_ITERATION; iteration++ )		/* start iteration */
	{
        ...
        for(i=0;i<length_total;i++){
            LLR_prev_turbo[i] = *(LLR_all_turbo_scr+i);
        }
        ...
        cout << "\n  get extrinsic information from decoder two \n";
        // Initial iteration this would be 0, as decoder 2 has not yet been given any input.
		...
		/* SCR termination scheme*/

        decision(LLR_all_turbo,length_total,LLR_all_turbo_scr_tmp);
        cout<<" Verifying if optimum has reached  " <<length_total<<"\n";
        int sc =0;
		random_deinterlvr_int(LLR_all_turbo_scr,LLR_all_turbo_scr_tmp,length_total,index_randomintlvr);

        for(i=0;i<length_total;i++){
            cout << *(LLR_all_turbo_scr+i)<<"  :  "<<LLR_prev_turbo[i] <<"\n";
            if(*(LLR_all_turbo_scr+i) != LLR_prev_turbo[i])
                sc++;
		}
		cout<<" \n  Number of sign changes   :  "<< sc << " : Frame size  : " << length_total <<"\n";
        cout<<"\n \n End of iteration : "<<iteration + 1<<"\n";

		if(sc <= Q*length_total){
            cout <<"\n  Algorithm converged... Exiting \n";
            break;
		}
						   ...
						   int length, int * index)
{
	...
	for (i=0; i<length; i++)
	{
        ...
        *(interleaverddata+(2*i+1)) = *(data_unintlvr+ (2*i));
		*(interleaverddata+ (2*i)) = *(data_unintlvr + (2*i +1));
	}
...
void randominterleaver_float(float *data_unintlvr, float *interleaverddata, int length, int *index)
{
	int i,j;
	int *index_random =  index ;
	int ln = 49*sizeof(float);
    cout<<" \n @@@@@@@@@@@@@@@ Length :  "<<length <<"   :  "  << ln <<"\n";


	cout<<"************* Uninterleaved array  \n";
	//cout<< *(data_unintlvr+55)<<"\n";

	for(j=0;j<length;j++){
        cout<<*(data_unintlvr+ j)<<"  ";
	}
    cout<<"\n\n";
	for (i=0; i<length; i++)
	{
         // Here, what is being done is, Interleaved data is nothing but uninterleaved data at random indices.
		//*(interleaverddata+i) = *(data_unintlvr+ (*(index_random+i)));
		*(interleaverddata+(2*i+1)) = *(data_unintlvr+ (2*i));
		*(interleaverddata+ (2*i)) = *(data_unintlvr + (2*i +1));

	}
    cout<<" ************* Interleaved data \n";
	for( i = 0 ; i < length ; i++)
		cout << *(interleaverddata+i) << "  ";
    cout << "\n";
}
/*---------------------------------------------------------------
FUNCTION:
...
void random_deinterlvr_int(int *data_unintlvr, int *interleaverddata, int length, int * index)
{
    cout << "Entering deinterleaver int \n";
	int i;
	int *index_random =  index;
   ...
   //printf("\n Process of deinterleaving! \n ");
	for (i=0; i<length; i++)
	{
        ...
        *(data_unintlvr+ (2*i)) = *(interleaverddata+(2*i+1));
		*(data_unintlvr + (2*i +1))  = *(interleaverddata+ (2*i)) ;
	}

	/*printf("\n After deinterleaving: \n");
	for (i=0; i<length; i++)
		printf(" %d ",	*(data_unintlvr+ i));*/
    cout << "Exiting deinterleaver successfully\n";
}
/*---------------------------------------------------------------
FUNCTION:
...
void random_deinterlvr_float(float *data_unintlvr, float *interleaverddata, int length, int *index)
{
    ...
    cout <<"****************** After deinterleaving ****************** \n";
	for (i=0; i<length; i++)
	{
		cout<< *(data_unintlvr + i) <<"  ";

	}
    cout<<"\n Exiting the deinterleaver float\n";
}
...
void encoderm_turbo(int *source, int *send_turbo, int len_info)
{
	...
	cout <<" ^^^^^^^^^^^^^^^^^^^^ withterminatebits \n";
	for(i =0;i<len_total;i++){
        cout<<*(with_terminate_bits+i) <<"  ";
	}
	cout <<"\n";
	...
	cout<<"Encoder 1 output \n";
	for(i=0;i< 2*len_total*sizeof(int) ; i++){
	   cout<<*(rsc1+i)<<"  ";
	}
	cout <<"\n";
...
void demultiplex(float *rec_turbo, int len_total, float *yk_turbo)
{
    ...
    //andominterleaver_float(info2, inted_info2, len_total, index_randomintlvr);
    randominterleaver_float(info2, inted_info2, len_total, index_randomintlvr);
    //randominterleaver_int(info2, inted_info2,len_total, index_randomintlvr);
	...
	free(info2);
	cout<<"################## Demultiplxing done.returning #################\n";
}
/*---------------------------------------------------------------
FUNCTION:
...
{
    cout <<"Entering LogMapdecoder   :  n_states = " <<n_states <<"\n";
	int i, j;
	...
	*(beta_Log + len_total*n_states) = 0;
	...
	cout << "Trace forward, compute Alpha_Log\n";
    ...
    cout <<"Trace backward, compute Beta_Log\n";
    ...
    cout <<"Compute the soft output,log-likelihood ratio of symbols in the frame\n";
    ...
    cout <<"Log map decoding done, freeing memory \n";
	free(alpha_Log);
	free(beta_Log);
	free(gama_Log);
	free(temp0);
